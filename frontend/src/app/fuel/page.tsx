...
const FUEL_TYPES = [
  { key: 'gasoline', label: 'АИ-95', price: 55.5 },
  { key: 'diesel', label: 'Дизель', price: 62.3 },
  { key: 'gas', label: 'Газ', price: 28.0 },
  { key: 'electric', label: 'Электро', price: 12.5 },
];
...
export default function FuelPage() {
  ...
  const handleCalculate = async () => {
    if (!origin.name || !dest.name) { toast.error('Введите маршрут'); return; }
    setLoading(true);
    try {
      ...
      toast.error(err?.response?.data?.message || 'Ошибка расчёта');
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="relative px-4 pt-14 max-w-lg mx-auto">
        ...
        <div className="flex items-center gap-3 mb-6">
          ...
          <div className="flex-1">
            <h1 className="text-2xl font-black text-white font-display">Калькулятор топлива</h1>
            <p className="text-gray-400 text-xs">Рассчитайте стоимость поездки</p>
          </div>
          <button onClick={() => setShowHistory(!showHistory)}
            className="px-3 py-2 rounded-xl bg-white/5 text-gray-400 text-sm hover:bg-white/10 flex items-center gap-1">
            <FaHistory size={12} /> История
          </button>
        </div>
          ...
          {showHistory && (
            ...
            >
              <div className="card p-4 space-y-2 max-h-60 overflow-y-auto">
                {history.length === 0 ? (
                  <p className="text-center text-gray-500 text-sm py-4">Нет истории расчётов</p>
                ) : history.map(h => (
                  <div key={h.id} className="flex items-center justify-between py-2 border-b border-white/5 last:border-0">
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="text-[10px] text-gray-500">{h.distanceKm} км · {h.fuelCost.toLocaleString()} ₽</p>
                    </div>
                  </div>
                ))}
              </div>
        ...
        <div className="card p-4 space-y-3 mb-4">
          ...
          <div className="relative">
            ...
            <input value={origin.name} onChange={e => { setOrigin(p => ({ ...p, name: e.target.value })); searchLocation(e.target.value, 'origin'); }}
              className="input-field pl-9 text-sm" placeholder="Откуда (город, адрес)" />
            <AnimatePresence>
          ...
          <div className="relative">
            ...
            <input value={dest.name} onChange={e => { setDest(p => ({ ...p, name: e.target.value })); searchLocation(e.target.value, 'dest'); }}
              className="input-field pl-9 text-sm" placeholder="Куда (город, адрес)" />
            <AnimatePresence>
          ...
          <div className="grid grid-cols-2 gap-2">
            <div>
              <label className="text-[10px] text-gray-500 uppercase tracking-wider">Расход (л/100км)</label>
            ...
            <div>
              <label className="text-[10px] text-gray-500 uppercase tracking-wider">Тип топлива</label>
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-white font-bold">{result.fuelCost.toLocaleString()} ₽</p>
                </div>
              </div>
            </div>
